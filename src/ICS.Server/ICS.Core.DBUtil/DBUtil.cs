...
using ICS.Core.DBUtil.BaseClass;
using ICS.Core.DBUtil.Interface;
using ICS.Core.Common;

namespace ICS.Core.DBUtil
{
    public class DBUtil
    {
        #region "Properties"
        ...
        {
            var func = ClassCollector.GetExecutingAssembly();
            var currentNamespace = func.Invoke().GetName().Name;
            var targetNamespace = string.Format("{0}.{1}", currentNamespace, _namespaceDB);
            _classes = ClassCollector.GetClasses(targetNamespace, func.Invoke().GetTypes());
        }
        ...
        public T[] GetTable<T>(DataTable dr) where T : TableBase, new()
        {
            var tableInfo = new T();
            ISetTable set = (ISetTable)tableInfo;
            return set.SetTable<T>(dr);
        }
